...
ANNkd_tree::~ANNkd_tree()		// tree destructor
{
    ...
    if (axis_weights != NULL) annDeallocPt(axis_weights);
}
...
ANNkd_tree::ANNkd_tree(			// basic constructor
	...
	int bs)				// bucket size
{   
    // make the default axis weight 1.0
    ANNpoint aw = annAllocPt(dd, 1.0);
   
    SkeletonTree(n, dd, aw, bs);
...
ANNkd_tree::ANNkd_tree(			// construct from point array
    ...
    int			n,		// number of points
    int			dd,		// dimension
    ANNpoint    aw,     // weights on each axis
    int			bs,		// bucket size
    ANNsplitRule	split)		// splitting method
{
    SkeletonTree(n, dd, aw, bs);		// set up the basic stuff

#if 1
    // Weight each coordinate of every point
    for (int i = 0; i < n; i++)
        for (int j = 0; j < dd; j++)
            pa[i][j] *= aw[j];
#endif
