...
int dgeev_driver(int n, double *A, double *evec, double *eval) {
    ...
    lwork = -1;
    F77(dgeev)(&jobvl, &jobvr, &n, Atmp, &lda, wr, wi, vl, &ldvl, vr, &ldvr, work_query, &lwork, &info);
    lwork = (int) work_query[0];
    work = malloc(sizeof(double) * lwork);

    /* Make the call to dgeev */
    F77(dgeev)(&jobvl, &jobvr, &n, Atmp, &lda, wr, wi, vl, &ldvl, vr, &ldvr, work, &lwork, &info);
 ...
 * eval: output array containing the eigenvalues
 *
 * Note: Returns complex outputs */

int dgeev_complex_driver(int n, double *A, 
                         double *evec_real, double *evec_imag, 
                         double *eval_real, double *eval_imag) 
{
    char jobvl = 'N';  /* Don't compute left eigenvectors */
    char jobvr = 'V';  /* Do compute right eigenvectors */
    int lda = n;
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
            if (A[i * n + j] != A[i * n + j]) {
                printf("[dgeev_complex_driver] Error: nan encountered\n");
                return 0;
            }
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < n; j++) {
            if (wi[i] == 0.0) {
                evec_real[i * n + j] = vr[i * n + j];
                evec_imag[i * n + j] = 0.0;
            } else if (i < n - 1 && wr[i] == wr[i+1] && wi[i] == -wi[i+1]) {
                evec_real[i * n + j] = vr[i * n + j];
                evec_imag[i * n + j] = vr[(i+1) * n + j];
            } else if (i > 0 && wr[i] == wr[i-1] && wi[i] == -wi[i-1]) {
                evec_real[i * n + j] = vr[(i-1) * n + j];
                evec_imag[i * n + j] = -vr[i * n + j];
            } else {
                printf("[dgeev_complex_driver] No case found!\n");
            }
        }
    }
...
// #else
    F77(lmdif1)(fcn, &m, &n, xvec, fvec, &tol, &info, iwa, wa, &lwa);
// #endif
...
// #else
    F77(lmdif)(fcn, &m, &n, xvec, fvec, &tol, &tol, &gtol, &maxfev, 
           &epsfcn, diag, &mode, &factor, &nprint, &info, &nfev, 
           fjac, &ldfjac, ipvt, qtf, wa1, wa2, wa3, wa4);
// #endif
...
void dgelss_driver(double *A, double *b, double *x, int m, int n, int nrhs) {
    if (m < n) {
        ...
        F77(dgelss)(&m, &n, &nrhs, Atmp, &lda, btmp, &ldb, 
...
void dgelsy_driver(double *A, double *b, double *x, int m, int n, int nrhs) {
    if (m < n) {
        ...
        F77(dgelsy)(&m, &n, &nrhs, Atmp, &lda, btmp, &ldb, jpvt,
			     ...
			     int m, int n, int nrhs) 
{
	...
	memcpy(btmp, b, sizeof(double) * m);
        ...
        /* Query to find a good size for the work array */
        F77(dgelsy)(&m, &n, &nrhs, A, &lda, btmp, &ldb, jpvt,
                &rcond, &rank, work, &lwork, &info);
...
void dgesv_driver(int n, double *A, double *b, double *x) {
    ...
    /* Make the FORTRAN call */
    F77(dgesv)(&n, &nrhs, Atmp, &lda, ipiv, btmp, &ldb, &info);
...
void matrix_lu(int n, double *A, double *LU, int *ipiv)
{
    ...
    F77(dgetrf)(&m, &n, At, &lda, ipiv, &info);
...
void matrix_lu_no_transpose(int n, double *A, double *LU, int *ipiv)
{
    ...
    /* Make calls to FORTRAN routines */
    F77(dgetrf)(&m, &n, Atmp, &lda, ipiv, &info);
...
void matrix_solve_lu(int n, double *LU, int *ipiv, double *b, double *x)
{
    ...
    F77(dgetrs)(&trans, &n, &nrhs, LU, &lda, ipiv, btmp, &ldb, &info);
...
void matrix_invert(int n, double *A, double *Ainv) {
    ...
    F77(dgetri)(&n, At, &lda, ipiv, work, &lwork, &info);
    if (info != 0)
...
void matrix_invert_inplace(int n, double *A) {
    ...
    /* Make calls to FORTRAN routines */
    F77(dgetrf)(&m, &n, A, &lda, ipiv, &info);
    F77(dgetri)(&n, A, &lda, ipiv, work, &lwork, &info);
...
int dgesvd_driver_vt(int m, int n, double *A, double *S, double *VT) {
    ...
    F77(dgesvd)(&jobu, &jobvt, &m, &n, AT, &lda, S, NULL, &ldu, V, &ldvt, work, &lwork, &info);
...
int dgesvd_driver(int m, int n, double *A, double *U, double *S, double *VT) {
    ...
    F77(dgesvd)(&jobu, &jobvt, &m, &n, AT, &lda, S, UT, &ldu, V, &ldvt, work, &lwork, &info);
...
void dgeqrf_driver(int m, int n, double *A, double *Q, double *R)
{
    ...
    F77(dgeqrf)(&m, &n, AT, &lda, tau, work, &lwork, &info);
...
void dgerqf_driver(int m, int n, double *A, double *R, double *Q)
{
    ...
    tau = (double *) malloc(sizeof(double) * tau_dim);
    work = (double *) malloc(sizeof(double) * lwork);
    F77(dgerqf)(&m, &n, AT, &lda, tau, work, &lwork, &info);
