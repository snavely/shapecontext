...
vec_t *compute_shape_context_driver(img_t *img_edge, 
...
int main_initialize_maps(int argc, char **argv) 
{
    ...
    if (argc != 6) {
	printf("Usage: %s initMaps <edge-image1.bmp> <edge-image2.bmp> "
...
#define NORMALIZE 1
    ...
    v1 = compute_shape_context_driver(img_edge1, 
				      NUM_RINGS, NUM_WEDGES, FACTOR, SIGMA, NORMALIZE);

    /* Second shape context */
    printf("Computing second shape context...\n");
    v2 = compute_shape_context_driver(img_edge2, 
				      NUM_RINGS, NUM_WEDGES, FACTOR, SIGMA, NORMALIZE);

    printf("Matching shape contexts [1==>2]...\n");
    dmap1to2 = match_shape_contexts(img_edge1, img_edge2, v1, v2, 
				    NUM_RINGS, NUM_WEDGES, DISTANCE_EUCLIDEAN);
    printf("Matching shape contexts [2==>1]...\n");
    dmap2to1 = match_shape_contexts(img_edge2, img_edge1, v2, v1,
				    NUM_RINGS, NUM_WEDGES, DISTANCE_EUCLIDEAN);
    ...
    dmap1to2_sym = 
	img_estimate_correspondence(img_edge2, img_edge1, dmap2to1, dmap1to2);

    dmap2to1_sym = 
	img_estimate_correspondence(img_edge1, img_edge2, dmap1to2, dmap2to1);
