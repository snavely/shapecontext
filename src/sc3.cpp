...
void write_features(const std::vector<feature_t> &features,
                    const img_t *img,
                    const char *keys_file, const char *descs_file)
{
    FILE *f_keys = fopen(keys_file, "w");
    FILE *f_descs = fopen(descs_file, "w");
    ...
    /* Headers */
    fprintf(f_keys, "# keys = %d, px = %0.3f, py = %0.3f\n", num_features,
            0.5 * img->w, 0.5 * img->h);
    ...
    for (int i = 0; i < num_features; i++) {
        ...
        fprintf(f_keys, "  %0.3f %0.3f 1.0 0.0\n", f.x, f.y);

        for (int j = 0; j < dim; j++) {
            fprintf(f_descs, "%0.6lf ", features[i].v.p[j]);
        }
...
int main_initialize_maps(int argc, char **argv) 
{
    ...
    // out_dpyr1to2 = argv[4];
    // out_dpyr2to1 = argv[5];
    const char *keys1_out = argv[4];
    const char *descs1_out = argv[5];
